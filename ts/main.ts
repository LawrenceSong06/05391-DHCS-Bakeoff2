...
class PivotBox {
	...
	diameter(){
		return this.pivot1.distance_to(this.pivot2);
	}
	rotation(){
	...
	render() : void {
		...
		const rotation = this.rotation() - 45;
...
window.addEventListener("load", (e: Event) => {
	...
	box.on("dragmove", (e)=> {
		const handler : DragHandler = (e as CustomEvent).detail.handler;
		const event : MouseEvent = (e as CustomEvent).detail.event;
		e.preventDefault();
		
		if (closer_to_pivot1){
			pivot_box.set_pivot1(event.offsetX, event.offsetY);
		}else {
			pivot_box.set_pivot2(event.offsetX, event.offsetY);
		}
		pivot_box.render();
